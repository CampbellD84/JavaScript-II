...
const total = runners.reduce((accValue, currValue) => accValue + currValue.donation, 0);
...
// Problem 1
// The event director needs the names and donation amounts to post on the event website
// return the full names and the donation back in an array and log it.

let donations = [];
runners.forEach(runner => donations.push(`${runner.first_name} ${runner.last_name}: ${runner.donation}`));
console.log(donations);

// Problem 2
// Event director needs each runner's company names w/o duplicates for banner. get a list of companies and log result. 
const companies = runners.reduce((accCo, currCo) => {
    if(accCo.indexOf(currCo.company_name) === -1) {
        accCo.push(currCo.company_name);
    }
    return accCo;
}, []);
...
// Problem 3
// A generous donor wants to add $50 to each runners' donation.  Please return a list of the updated donations with the donor's contribution and log it.

let newDonations = [];
runners.map(runner => newDonations.push(runner.donation + 50));
